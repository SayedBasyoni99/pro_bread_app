...
class GetCategoriesCubit extends Cubit<GetCategoriesState> {
  ...
  GetCategoriesCubit(this.getCategoriesUseCase) : super(const GetCategoriesInitialState());

  List<Category> data = <Category>[];

  Future<void> fGetCategories() async {
    emit(const GetCategoriesLoadingState());
    final Either<Failure, GetCategoriesResponse> eitherResult = await getCategoriesUseCase(NoParams());
    eitherResult.fold((Failure failure) {
      emit(GetCategoriesErrorState(message: failure.message?? 'pleaseTryAgainLater'));
